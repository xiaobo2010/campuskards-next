...
import type { BattlePackDrop, BattleReportEvent, GameOverPayload } from "@/types";
import { cn } from "@/lib/utils";
import {
  PackOpeningAnimation,
  type PackResultCard,
} from "@/components/game/pack-opening-animation";
import { battlePackToResultCards, packAnimationId } from "@/lib/battle-rewards";
...
export default function BattleResultScreen({
  ...
  const [animation, setAnimation] = useState<PackAnimationState | null>(null);

  const startPackOpen = (pack: BattlePackDrop) => {
    setAnimation({
      cards: battlePackToResultCards(pack),
      packName: pack.name,
      packId: packAnimationId(pack.pack_id),
    });
  };
            ...
            {isWin && rewards && (rewards.ink > 0 || rewards.packs.length > 0) && (
              <section className="rounded-2xl border border-amber-700/40 bg-amber-950/20 p-4 space-y-3">
                ...
                {rewards.packs.map((pack) => (
                  <div key={`${pack.pack_id}-${pack.name}`} className="space-y-2">
          ...
          <div className="shrink-0 flex flex-col gap-2 pt-2 pb-safe">
            {matchId && (
              <Button
                asChild
                variant="outline"
                className="w-full border-zinc-600 bg-zinc-900/50"
              >
