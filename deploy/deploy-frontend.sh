...
# 可独立执行：依赖安装 → 构建 → standalone 静态复制 → 服务重启
# ============================================================================
set -euo pipefail
...
if [[ -z "${NEXT_PUBLIC_API_URL:-}" ]]; then
  info "NEXT_PUBLIC_API_URL 未设置，将使用 next.config.js 中的 BACKEND_URL 默认值"
  if confirm "设置生产环境 API URL?"; then
    read -r -p "$(echo -e "${YELLOW}?${NC} API URL [https://api.your-domain.com]: ")" api_url
    export NEXT_PUBLIC_API_URL="${api_url:-https://api.your-domain.com}"
  fi
fi
